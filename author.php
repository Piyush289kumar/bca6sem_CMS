        ...
        <div class="row">
            <div class="col-md-8">
              ...
              <div class="post-container">
                    <?php
                    if (isset($_GET['aid'])) {
                        $aid_by_addbar = $_GET['aid'];
                    }
                    $sql_total_post_record = "SELECT * from post JOIN user
                                              ON post.author = user.user_id
                                              WHERE post.author = {$aid_by_addbar}";
                    ...
                    ?>
                    <h2 class="page-heading"><?php echo "We're Love {$post_row_num['username']} CMS's" ?></h2>
                    <?php
                        include("config.php");
                    ...
                    <?php
                        ...
                        if (mysqli_num_rows($result_sql_total_post_record) > 0) {
                            ...
                            if ($page_index_by_addbar > 1) {
                                echo ("<li><a href='$hostname/author.php?aid={$aid_by_addbar}&page_index=" . ($page_index_by_addbar - 1) . "'>Prev</a></li>");
                            }
                            for ($i = 1; $i <= $total_page; $i++) {
                                ...
                                echo ("<li class=$active_page><a href='$hostname/author.php?aid={$aid_by_addbar}&page_index=$i'>$i</a></li>");
                            }
                            if ($page_index_by_addbar < $total_page) {
                                echo ("<li><a href='$hostname/author.php?aid={$aid_by_addbar}&page_index=" . ($page_index_by_addbar + 1) . "'>Next</a></li>");
                            }
                            echo ("</ul>");
                        }
                        ?>
